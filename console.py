...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, line):
        ...
        elif method == self.cmd_list[0]:
            self.do_all(class_list[0])

        elif method == self.cmd_list[2]:
            print(self.class_count(class_list[0]))

        elif method == self.cmd_list[4]:
            self.do_show(class_list[0] + " " + class_id[0])

        elif method == self.cmd_list[3]:
    ...
    def do_update(self, command):
        """Update an instance based on class & id by adding/updating attr."""
